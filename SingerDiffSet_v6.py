...
#  v6: simplified the approach by employing the so-called zeta-generated perfect
# difference set (see "On Perfect difference Sets" by Halberstan and Laxton).
# Only seems to work if q is prime so far, but is very fast in those cases.
...
ExtendedField = {}
ExtendedField[0] = Poly(1, x, modulus=p)
ExtendedField[1] = Poly(x, x, modulus=p)
...
for i in range(2, v):
    ...
    #print 'EF[{0}] = {1}'.format(i, ExtendedField[i])
    if ExtendedField[i].degree() < 2:
